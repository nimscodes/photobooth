  ...
  return (
    <div className="w-full">
            ...
            <button key={day.toISOString()} type="button" disabled={unavail}
              ...
              className={["aspect-square rounded-lg text-sm font-medium transition-all",
                unavail ? "text-white/20 cursor-not-allowed line-through"
                  : sel ? "bg-[#8B5CF6] text-[#0B1020] font-bold shadow-lg"
                  : tod ? "ring-1 ring-[#8B5CF6]/60 text-white hover:bg-white/10"
      ...
      <div className="mt-4 flex flex-wrap gap-4 text-xs text-white/50">
        <span className="flex items-center gap-1.5"><span className="w-3 h-3 rounded bg-[#8B5CF6] inline-block" /> Selected</span>
